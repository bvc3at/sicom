...
fn compress_pack(
        ...
        if is_image {
            match compress_image_file(&mut file, &file_name, image_quality) {
                Ok((compressed_data, original_size, compressed_size)) => {
                    ...
                    zip_writer.write_all(&compressed_data)
                        .with_context(|| format!("Failed to write compressed image: {}", file_name))?;
                    
                    processed_images += 1;
                    total_original_size += original_size;
                    total_compressed_size += compressed_size;
                    
                    println!("  Compressed: {} bytes -> {} bytes ({:.1}% reduction)", 
    ...
    if total_original_size > 0 {
        println!("Total image size reduction: {} bytes -> {} bytes ({:.1}% reduction)",
...
fn compress_image_file(
    ...
    let compressed_data = match format {
        ...
        ImageFormat::Png => {
            let mut buffer = Vec::new();
            // PNG compression level is 0-9, map quality 1-100 to compression level
            let _compression_level = ((100 - quality) as f32 / 100.0 * 9.0) as u8;
            // Note: image crate PNG encoder doesn't expose compression level directly
        ...
        ImageFormat::WebP => {
            let mut buffer = Vec::new();
            // WebP encoder in image crate doesn't support quality setting in the same way
            // Let's use lossless encoding for now, or fall back to original
            if quality >= 95 {
                let encoder = image::codecs::webp::WebPEncoder::new_lossless(&mut buffer);
                img.write_with_encoder(encoder)
                    .with_context(|| "Failed to encode WebP")?;
            } else {
                // For lower quality, fall back to original data as image crate doesn't support lossy WebP encoding
                return Err(anyhow!("Lossy WebP compression not supported by image crate"));
            }
            buffer
        },
...
mod tests {
    ...
    fn test_quality_validation() {
        ...
        assert!(result.is_err());
        assert!(!result.unwrap_err().to_string().contains("quality must be between"));
    }
