...
use mp3lame_encoder::{Builder, FlushNoGap, InterleavedPcm, Bitrate};
use std::path::Path;
            ...
            Ok(decoded) => {
                ...
                match decoded {
                    ...
                    AudioBufferRef::U8(buf) => {
                        ...
                        if buf.spec().channels.count() == 1 {
                            for &sample in buf.chan(0) {
                                let f_sample = (sample as f32 - 128.0) / 128.0;
                            ...
                            let right = buf.chan(1);
                            for (l, r) in left.iter().zip(right.iter()) {
                                let f_l = (*l as f32 - 128.0) / 128.0;
                                let f_r = (*r as f32 - 128.0) / 128.0;
                                audio_data.push(f_l);
                                audio_data.push(f_r);
                            }
                        }
                    ...
                    AudioBufferRef::U16(buf) => {
                        ...
                        if buf.spec().channels.count() == 1 {
                            for &sample in buf.chan(0) {
                                let f_sample = (sample as f32 - 32768.0) / 32768.0;
                                audio_data.push(f_sample);
                            }
                        } else {
                            let left = buf.chan(0);
                            let right = buf.chan(1);
                            for (l, r) in left.iter().zip(right.iter()) {
                                let f_l = (*l as f32 - 32768.0) / 32768.0;
                                let f_r = (*r as f32 - 32768.0) / 32768.0;
                                audio_data.push(f_l);
                                audio_data.push(f_r);
                            }
                        }
                    ...
                    AudioBufferRef::S16(buf) => {
                        ...
                        if buf.spec().channels.count() == 1 {
                            ...
                            let right = buf.chan(1);
                            for (l, r) in left.iter().zip(right.iter()) {
                                let f_l = *l as f32 / 32768.0;
                                let f_r = *r as f32 / 32768.0;
                                audio_data.push(f_l);
                                audio_data.push(f_r);
                            }
                        }
                    ...
                    AudioBufferRef::S32(buf) => {
                        ...
                        if buf.spec().channels.count() == 1 {
                            for &sample in buf.chan(0) {
                                let f_sample = sample as f32 / 2147483648.0;
                                audio_data.push(f_sample);
                            }
                        } else {
                            let left = buf.chan(0);
                            let right = buf.chan(1);
                            for (l, r) in left.iter().zip(right.iter()) {
                                let f_l = *l as f32 / 2147483648.0;
                                let f_r = *r as f32 / 2147483648.0;
                                audio_data.push(f_l);
                                audio_data.push(f_r);
                            }
                        }
...
fn compress_mp3_file(data: &[u8], quality: u8) -> Result<Vec<u8>> {
    ...
    builder.set_brate(target_bitrate)
    ...
    const SAMPLES_PER_FRAME: usize = 1152; // MP3 frame size
    ...
    let final_buffer: Vec<u8> = mp3_buffer.into_iter().map(|b| unsafe { b.assume_init() }).collect();
...
mod tests {
    ...
    fn test_detect_audio_format() {
        ...
        assert_eq!(detect_audio_format("test.MP3"), Some(AudioFormat::Mp3));
        assert_eq!(detect_audio_format("Audio/song.mp3"), Some(AudioFormat::Mp3));
